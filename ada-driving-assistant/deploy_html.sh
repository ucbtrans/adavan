...
# v2: inject API URL + v2.2 label
$PYTHON -c "
api_url = '$API_URL'
content = open('templates/index.html', encoding='utf-8').read()
content = content.replace(\"window.ADA_API_BASE || ''\", \"'\" + api_url + \"'\")
content = content.replace('<title>ADA Driving Assistant</title>', '<title>ADA Driving Assistant v2.2</title>')
content = content.replace('<h1>ADA <span>Driving Assistant</span></h1>', '<h1>ADA <span>Driving Assistant</span> <span style=\"font-size:0.7rem;color:var(--muted);font-weight:400;\">v2.2</span></h1>')
